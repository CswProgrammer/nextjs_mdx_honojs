    ...
    .post(
        ...
        async (c) => {
            try {
                ...
                // ✅ 修复 null 的 slug 问题并加类型断言
                const filteredBody = Object.fromEntries(
                    Object.entries(body).map(([key, val]) => [key, val === null ? undefined : val]),
                ) as PostCreateInput;
                const result = await createPostItem(filteredBody);
    ...
    .patch(
        ...
        async (c) => {
            try {
                ...
                const result = await updatePostItem(params.id, filteredBody);
                return c.json(result, 200);
            } catch (error) {
